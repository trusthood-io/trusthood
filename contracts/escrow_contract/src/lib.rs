//! # TrustHoodEscrow — Soroban Smart Contract
//!
...
use trusthood_shared::auth as shared_auth;
...
impl EscrowContract {
    ...
    /// 1. Deadline check — rejects expired meta-transactions.
    /// 2. Nonce check — enforces strictly monotonically increasing nonces per
    ///    signer to prevent replay attacks.
    /// 3. Signature verification — Ed25519 signature over the canonical message
    ///    must verify against the signer's Stellar account public key.
    ///
    /// Canonical message layout (all integers little-endian):
    ///   nonce (8 bytes) || deadline (8 bytes)
    ///   The function_name and function_args are not signed in this version;
    ///   the signer's Address auth covers them implicitly via require_auth.
    ///
    /// On success the nonce is advanced and the meta-transaction is accepted.
    pub fn execute_meta_transaction(
        ...
        // ── 2. Nonce check ───────────────────────────────────────────────────
        // Enforces nonce > last_nonce and persists the new nonce, preventing
        // replay attacks without relying on the host auth context.
        ContractStorage::_validate_and_update_nonce(&env, &meta_tx.signer, meta_tx.nonce)?;

        // ── 3. Signature verification ────────────────────────────────────────
        // Canonical message: nonce (8 bytes LE) || deadline (8 bytes LE)
        // This binds the signature to the exact nonce and deadline so neither
        // can be tampered with by the relayer after signing.
        let mut msg = [0u8; 16];
        msg[0..8].copy_from_slice(&meta_tx.nonce.to_le_bytes());
        msg[8..16].copy_from_slice(&meta_tx.deadline.to_le_bytes());

        // Extract the signer's 32-byte Ed25519 public key from the Address XDR.
        // A Stellar account Address XDR encoding is:
        //   4-byte type discriminant (PublicKeyType::PublicKeyTypeEd25519)
        //   + 32-byte raw Ed25519 public key
        // We take the trailing 32 bytes.
        use soroban_sdk::xdr::ToXdr;
        let signer_xdr = meta_tx.signer.to_xdr(&env);
        let xdr_len = signer_xdr.len();
        if xdr_len < 32 {
            return Err(EscrowError::E3);
        }
        ...
        for i in 0..32u32 {
            pubkey_arr[i as usize] = signer_xdr.get(key_start + i).unwrap_or(0);
        }
        let pubkey: BytesN<32> = BytesN::from_array(&env, &pubkey_arr);

        // ed25519_verify panics (via Soroban host trap) on invalid signature,
        // which rolls back the transaction and charges the caller for the work done.
        env.crypto().ed25519_verify(
            &pubkey,
            &soroban_sdk::Bytes::from_slice(&env, &msg),
            &meta_tx.signature,
